<x-slot name="head">
    <title>{{ env('APP_NAME').' - Update profile' }}</title>
    ...
    <section class="border-b container flex-grow lg:flex lg:flex-row lg:py-10 max-w-[1200px] mx-auto py-5 w-full">
        ...
        <section class="gap-3 grid grid-cols-1 max-w-[1200px] pb-10 px-5 w-full">
            <div class="py-5">
                ...
                <form class="flex flex-col gap-3 w-full" wire:submit.prevent="updateName">
                    <div class="flex flex-col w-full">
                        ...
                        <input
                            wire:model="name" name="name" required
                            class="border lg:w-1/2 px-4 py-2 w-full" type="text" placeholder="John Doe" value="{{ old('name', auth()->user()->name) }}">
                    </div>

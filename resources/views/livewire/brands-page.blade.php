...
<div>
    ...
    <div class="gap-3 max-w-[1200px] mx-auto py-5 px-5">
        <h1 class="text-2xl w-full font-semibold">{{ $brand->name }} products</h1>
    </div>
    @if($products->count()>0)
                            ...
                            <div>
                                ...
                                <div class="flex items-center">
                                    <p class="text-gray-400 text-sm">
                                        brand: <a href="/brands/{{ $product->brand->slug }}"
                                                     class="hover:text-blue-700 underline">{{ $product->brand->name }}</a>
                                    </p>
                                </div>
                                <div>
                                    <button
    ...
    @else
        <section class="mt-20 px-4 mb-32">
            <div class="flex flex-col">
                <p class="font-bold text-3xl text-center text-red-600 mb-10">! Currently no products for {{ $brand->name }} </p>
                <div class="text-center">
                    ...
                    @foreach($brands as $index => $brand)
                        <a href="/brands/{{ $brand->slug }}"
                           class="link text-blue-500 hover:underline">{{ $brand->name }}</a>
                        @if($index < $brandCount - 2)
                            ,
                        @elseif($index === $brandCount - 2)
                            and
                        @endif
                    @endforeach
                </div>
